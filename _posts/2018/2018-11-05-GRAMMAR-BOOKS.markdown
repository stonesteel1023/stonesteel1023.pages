...
>> 만약 관계형 데이터베이스에 대해서 전혀 모르거나, SQL 문법만 알고 있다면 이 책으로 시작해보시길 권해드립니다. 그리고 관계형 데이터베이스를 저처럼 대충 사용했던 개발자라면 이 책을 통해서 관계형 데이터베이스에 대한 개념과 관점을 새롭게 만들어보는 시간을 가져보면 좋습니다. ### 8. 아는 만큼 보이는 데이터베이스 설계와 구축, 이춘식 (지은이), 한빛미디어, 2008>>를 함께 읽으면 이론과 설계에 관한 체계적인 시각을 가질 수 있습니다.
>
...
### 멀티플레이어 게임 프로그래밍 - 네트워크 게임의 아키텍처를 만드는 기술, 조슈아 글레이저, 산제이 마드하브 (지은이), 장준혁 (옮긴이), 길벗, 2017
>> 이 책을 통해서 배웠던 내용을 복습하시면 좋습니다. 그럼에도 불구하고, 이 책에 나온 정도의 간단한 소켓 프로그래밍부터 시작하지 않고선, 어떤 것도 해볼 수 없기 때문에 처음이라면 이 책과 함께 하시길 추천합니다.

### 수학이 필요한 순간 - 인간은 얼마나 깊게 생각할 수 있는가, 김민형 (지은이), 인플루엔셜(주), 2018
>> 수학에 관심있는 분들은 참고하세요.
...
### 컴퓨터 보안과 암호, 윌리엄 스탈링스 (지은이), 최용락 (옮긴이), 그린, 2011 / 처음 배우는 암호화 ### 기초 수학부터 양자 컴퓨터 이후까지, 암호학의 현재와 미래, 장필리프 오마송 (지은이), 류광 (옮긴이), 한빛미디어, 2018
...
### 자바로 배우는 리팩토링 입문 - 건강한 코드로 소프트웨어 체질을 개선하자, 유키 히로시 (지은이), 서수환 (옮긴이), 길벗
>> 만약 자바 개발자라면 이 책을 권합니다.

### 리팩토링 - 루비 에디션 - 버그 없이 프로그램을 개선시키는 최고의 방법!, 마틴 파울러, 켄트 벡, 제이 필즈, 셰인 하비 (지은이), 김지원 (옮긴이), 제이펍, 2011
...
### 객체지향 자바스크립트 3/e - 객체지향 프로그래밍의 기초부터 함수.객체.프로토타입까지, 베드 안타니, 스토얀 스테파노프 (지은이), 류영선 (옮긴이), 에이콘출판), 2017
>> 번역의 질이나 내용이 나쁘지 않기 때문에 충분히 좋은 대안이 될 수 있습니다.

### Introduction to Algorithms - Third Edition, 토머스 코멘, 찰스 레이서손, 로날드 리베스트, 클리포드 스타인 (지은이), 문병로, 심규석, 이충세 (옮긴이), 한빛아카데미, 2014
>> 만약 이론적인 측면에서 알고리즘을 접근하고자 한다면 권하며, 수학을 기반으로 한 책은

### The Art of Computer Programming, 도널드 커누스 (지은이), 류광 (옮긴이), 한빛미디어 (이하 TAOCP), 2006
...
### 전문가를 위한 C++ - C++14, 제네릭람다, STL, 템플릿, 디자인패턴, 객체지향의 원리를 익히는 확실한 방법, 개정3판, 마크 그레고리 (지은이), 권오인 (옮긴이), 한빛미디어
>> 만약 C++ 언어에 대한 넓고 깊은 설명이 필요하시다면 ### 8. 를 권하며,

### The C++ Programming Language (Fourth Edition)
>> C++ 언어의 철학이 궁금하시면 이 책을 읽어보시면 됩니다.

### 이것이 C++이다 - 강의 현장을 그대로 옮긴 C++ 입문서, 최호성 (지은이), 한빛미디어, 2016
...
### C로 쓴 자료구조론 - 2판, Ellis Horowitz (지은이), 이석호 (옮긴이), 교보문고(교재, 2008
...
### C 포인터의 이해와 활용 - 메모리 관리의 핵심 기술, 리차드 리스(지은이), 조인중, 강성용(옮긴이), 제이펍, 2013
>> 만약 C언어 문법 중 포인터에 대한 내용이 필요하면 도움될 것이며, C언어로 작은 프로젝트를 진행하고 싶다면

### 단계별 프로젝트 중심의 C프로그래밍 파워 업그레이드, 윤성우 (지은이), 오렌지미디어, 2008
>>가 좋습니다. C언어 문법을 배웠던 경험이 있다면

 ### C 기초 플러스 6판 - 최신 C11 버전 포함, 개정판, Stephen Prata (지은이),윤성일, 이선민, 조혜란 (옮긴이), 성안당, 2017
 >>을 참고하는게 도움이 됩니다. 그런데 C 기초 플러스 6판의 경우 몇군데 컴파일 오류가 발생하기 때문에 출판사의 정오표를 꼭 확인하면서 읽기를 권합니다.

### K. N. King의 ### 8. C Programming : A Modern Approach, 2008
>> C언어 만큼은 원서로 공부해야겠다면 하나만 보셔도 충분합니다. 개인적으로 Kerninghan & Ritchie의 ### 8. The C Programming Language, 1988>>보다 K. N. King의 책을 권합니다.

### 스타트 스프링 부트 - 초급 개발자들을 위한 가볍고 넓은 스프링 부트, 구멍가게 코딩단 (지은이), 남가람북스, 2017
...
### Functional Programming in Java 8 - 자바 8 람다의 힘, 벤컷 수브라마니암 (지은이), 장독대 (옮긴이), 루비페이퍼, 2014
